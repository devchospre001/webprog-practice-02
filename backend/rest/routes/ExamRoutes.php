...
Flight::route('GET /employee/@employee_id', function ($employee_id) use ($service) {
    try {
        $employee = $service->get_employee($employee_id);
        if ($employee) {
            Flight::json($employee);
        } else {
            Flight::json(['message' => 'Employee not found'], 404);
        }
    } catch (Exception $e) {
        Flight::json(['message' => 'Error occurred while fetching employee', 'error' => $e->getMessage()], 500);
    }
});

Flight::route('GET /employees/performance', function () use ($service) {
    /** TODO
    ...
    try {
        ...
        Flight::json(['message' => 'Error occurred while fetching data', 'error' => $e->getMessage()], 500);
    }
});

Flight::route('DELETE /employee/delete/@employee_id', function ($employee_id) use ($service) {
    /** TODO
    ...
    try {
        ...
        Flight::json(["message" => "Employee with ID $employee_id deleted successfully."]);
    } catch (Exception $e) {
        Flight::json(['message' => 'Error occurred while deleting employee', 'error' => $e->getMessage()], 500);
    }
});

Flight::route('PUT /employee/edit/@employee_id', function ($employee_id) use ($service){
    /** TODO
    ...
    try {
        $updatedEmployee = $service->edit_employee($employee_id, $data);
        Flight::json($updatedEmployee);
    } catch (Exception $e) {
        Flight::json(['message' => 'Error occurred while updating employee', 'error' => $e->getMessage()], 500);
    }
});

Flight::route('GET /orders/report', function () use ($service) {
    /** TODO
...
Flight::route('GET /order/details/@order_id', function ($order_id) use ($service){
    /** TODO
